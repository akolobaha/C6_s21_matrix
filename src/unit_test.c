...
START_TEST(s21_inverse_matrix_4) {
  ...
  matrix_t A, res_1, res_2;
  s21_create_matrix(3, 3, &A);
  s21_create_matrix(3, 3, &res_1);
  for (int i = 0; i < A.rows; i++) {
  ...
  for (int i = 0; i < A.rows; i++) {
    for (int j = 0; j < A.columns; j++) {
      res_1.matrix[i][j] = n[i][j];
    }
  }
  int res = s21_inverse_matrix(&A, &res_2);
  ck_assert_int_eq(res, 0);
  res = s21_eq_matrix(&res_1, &res_2);
...
Suite *s21_matrix_suite() {
  ...
  tcase_add_test(tcase_core, s21_transpose_1_1);
